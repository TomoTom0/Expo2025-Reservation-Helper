    ...
    <div class="ytomo-entrance-reservations">
      ...
      <div v-if="selectedDate" class="ytomo-reservation-actions">
        <button
          ...
          :class="{ 'disabled': !isReservationButtonEnabled, 'cancel-mode': entranceReservationStore.isReservationRunning }"
          :disabled="!isReservationButtonEnabled"
          @click="executeReservation"
        >
          {{ entranceReservationStore.isReservationRunning ? '予約中断' : reservationButtonText }}
        </button>
      </div>
    </div>
  </div>
...
// 選択解除コールバックを設定
entranceReservationStore.setClearSelectionCallback(clearTimeSlotSelection)
...
const executeReservation = async () => {
  ...
  await entranceReservationStore.executeReservation(
    selectedTimeSlots,
    selectedDate.value,
    existingReservationId,
    originalReservationInfo,
    isReservationButtonEnabled.value,
    timeSlotInfo,
    selectedSchedule.value
  )
}

// 指定時間の選択済み時間帯の空き状況を取得
...
onMounted(async () => {
  ...
  logger.info('[ENTRANCE:EntranceTab] マウント開始')
  ...
  try {
    ...
    logger.info('[ENTRANCE:EntranceTab] スケジュールデータ取得開始', { year, month })
    await ticketsStore.getEntranceScheduleData(year, month, false)

    // デフォルト選択の初期化（選択中の入場予約がある場合は自動設定）
    initializeDefaultSelection()

    // 選択日付が設定されている場合は時間帯データを読み込み
    if (selectedDate.value) {
      await loadTimeSlotsForDate(selectedDate.value)
    }

    logger.info('[ENTRANCE:EntranceTab] マウント完了', {
      selectedDate: selectedDate.value,
      selectedSchedule: !!selectedSchedule.value,
      timeSlotsCount: timeSlots.value.length
    })
  } catch (error) {
    logger.error('[ENTRANCE:EntranceTab] マウントエラー', error)
  }
})
</script>
