...
/* harmony import */ var _reservation_starter__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(369);
/**
 * 監視サービス
 * API呼び出し・空き検知・自動予約実行
 */
...
class MonitoringService {
    ...
    async executeReservation(target) {
        ...
        try {
            // 共通の自動予約開始関数を呼び出し
            const success = await (0,_reservation_starter__WEBPACK_IMPORTED_MODULE_2__/* .startAutomationReservation */ .U)(target.pavilionCode, target.pavilionName, target.timeSlot, target.timeSlot);
            if (success) {
                // 監視成功通知を送信
                this.sendNotificationToDialog('info', `監視成功: ${target.pavilionName} ${target.timeSlot} の空きを検知し予約開始`);
            }
            else {
                throw new Error('自動予約開始に失敗しました');
            }
        }
        catch (error) {
...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   U: () => (/* binding */ startAutomationReservation)
/* harmony export */ });
 ...
 */
async function startAutomationReservation(pavilionCode, pavilionName, timeSlot, timeDisplay) {
...
/* harmony import */ var _reservation_starter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(369);
/**
 * 即時予約機能
 * 空き時間帯での即座予約実行
 */
...
class ImmediateReservationService {
    ...
    static async executeReservation(pavilionCode, pavilionName, timeSlot, timeDisplay) {
        ...
        try {
            // 共通の自動予約開始関数を呼び出し
            return await (0,_reservation_starter__WEBPACK_IMPORTED_MODULE_0__/* .startAutomationReservation */ .U)(pavilionCode, pavilionName, timeSlot, timeDisplay);
        }
        catch (error) {
...
function checkNextRouter() {
    try {
        ...
        console.log('Content Script - window.next:', window.next);
 ...
 * @returns 実行成功の可否
 */
function executeRouterPush(url) {
    try {
...
const RouterUtils = {
    ...
    logInfo() {
        ...
        console.log('Content Script context - window.next:', window.next);
    }
};
...
const init_page = () => {
    ...
    const createPavilionFAB = () => {
        ...
        const btnDebug = createSubButton('デバッグ', 'btn-debug');
        // DOM構築
        ...
        subActionsContainer.appendChild(btnDebug);
    ...
    document.addEventListener("click", (event) => {
        if (event.target?.matches?.("button.ext-ytomo, button.ext-ytomo *, button.pavilion-sub-btn, button.pavilion-sub-btn *")) {
            ...
            else if (target && target.classList.contains("btn-debug")) {
                // デバッグ
                console.log('🔧 デバッグボタンがクリックされました');
                showDebugDialog().catch(error => {
                    console.error('❌ デバッグダイアログエラー:', error);
                    showMobileErrorDialog('デバッグエラー', error);
                });
            }
        }
    });
...
const showDebugDialog = async () => {
    console.log('🔧 デバッグダイアログを表示します');
    // 既存のダイアログがある場合は削除
    const existingDialog = document.getElementById('debug-dialog');
    if (existingDialog) {
        existingDialog.remove();
    }
    // ダイアログ全体のコンテナ
    const dialogOverlay = document.createElement('div');
    dialogOverlay.id = 'debug-dialog';
    dialogOverlay.className = 'ytomo-dialog overlay';
    // ダイアログコンテンツ
    const dialogContent = document.createElement('div');
    dialogContent.className = 'ytomo-dialog container debug-dialog';
    // ヘッダー
    const header = document.createElement('div');
    header.className = 'ytomo-dialog header';
    // タイトル
    const title = document.createElement('h2');
    title.className = 'ytomo-dialog title';
    title.textContent = 'デバッグツール';
    // 閉じるボタン（右上）
    const closeButton = document.createElement('button');
    closeButton.className = 'ytomo-dialog close-button';
    ...
    searchSection.className = 'debug-section';
    const searchTitle = document.createElement('h3');
    searchTitle.textContent = '🔍 直接API検索';
    const searchDescription = document.createElement('p');
    searchDescription.textContent = '公式サイトの /api/d/events を直接呼び出してパビリオン情報を取得します。';
    // 検索入力フィールド
    const inputGroup = document.createElement('div');
    inputGroup.className = 'debug-input-group';
    const searchLabel = document.createElement('label');
    searchLabel.textContent = '検索クエリ:';
    const searchInput = document.createElement('input');
    searchInput.type = 'text';
    searchInput.className = 'debug-search-input';
    searchInput.placeholder = 'パビリオン名（例: 日本館）';
    searchInput.value = '日本';
    const searchButton = document.createElement('button');
    searchButton.className = 'debug-search-button';
    searchButton.textContent = '検索実行';
    inputGroup.appendChild(searchLabel);
    inputGroup.appendChild(searchInput);
    inputGroup.appendChild(searchButton);
    // パビリオンコード入力グループ
    const pavilionGroup = document.createElement('div');
    pavilionGroup.className = 'debug-input-group';
    const pavilionLabel = document.createElement('label');
    pavilionLabel.textContent = 'パビリオンコード:';
    const pavilionInput = document.createElement('input');
    pavilionInput.type = 'text';
    pavilionInput.className = 'debug-pavilion-input';
    pavilionInput.placeholder = 'パビリオンコード（例: H1H9）';
    pavilionInput.value = 'H1H9';
    const schedulesButton = document.createElement('button');
    schedulesButton.className = 'debug-schedules-button';
    schedulesButton.textContent = '全時間帯取得';
    pavilionGroup.appendChild(pavilionLabel);
    pavilionGroup.appendChild(pavilionInput);
    pavilionGroup.appendChild(schedulesButton);
    // 時間帯予約グループ
    const reservationGroup = document.createElement('div');
    reservationGroup.className = 'debug-input-group';
    const timeLabel = document.createElement('label');
    timeLabel.textContent = '時間帯:';
    const timeInput = document.createElement('input');
    timeInput.type = 'text';
    timeInput.className = 'debug-time-input';
    timeInput.placeholder = '時間（例: 1400）';
    timeInput.value = '1400';
    const reserveButton = document.createElement('button');
    reserveButton.className = 'debug-reserve-button';
    reserveButton.textContent = '時間帯予約';
    reserveButton.style.backgroundColor = '#ff4444';
    reserveButton.style.color = 'white';
    reservationGroup.appendChild(timeLabel);
    reservationGroup.appendChild(timeInput);
    reservationGroup.appendChild(reserveButton);
    // 結果表示エリア
    const resultsArea = document.createElement('div');
    resultsArea.className = 'debug-results-area';
    const resultsTitle = document.createElement('h4');
    resultsTitle.textContent = '📊 検索結果';
    const resultsDisplay = document.createElement('pre');
    resultsDisplay.className = 'debug-results-display';
    resultsDisplay.textContent = '検索結果がここに表示されます...';
    resultsArea.appendChild(resultsTitle);
    resultsArea.appendChild(resultsDisplay);
    // 画面遷移テストセクション
    const navigationSection = document.createElement('div');
    navigationSection.className = 'debug-section';
    const navTitle = document.createElement('h3');
    navTitle.textContent = '🚀 画面遷移テスト';
    const navDescription = document.createElement('p');
    navDescription.textContent = 'Next.js Routerを使用した画面遷移をテストします。';
    // パビリオンコード入力
    const navInputGroup = document.createElement('div');
    navInputGroup.className = 'debug-input-group';
    const navLabel = document.createElement('label');
    navLabel.textContent = 'パビリオンコード:';
    const navInput = document.createElement('input');
    navInput.type = 'text';
    navInput.className = 'debug-nav-input';
    navInput.placeholder = 'パビリオンコード（例: PAVILION001）';
    navInput.value = 'H1H9';
    const navButton = document.createElement('button');
    navButton.className = 'debug-nav-button';
    navButton.textContent = '画面遷移テスト';
    navInputGroup.appendChild(navLabel);
    navInputGroup.appendChild(navInput);
    navInputGroup.appendChild(navButton);
    // セクション構築
    searchSection.appendChild(searchTitle);
    searchSection.appendChild(searchDescription);
    searchSection.appendChild(inputGroup);
    searchSection.appendChild(pavilionGroup);
    searchSection.appendChild(reservationGroup);
    searchSection.appendChild(resultsArea);
    navigationSection.appendChild(navTitle);
    navigationSection.appendChild(navDescription);
    navigationSection.appendChild(navInputGroup);
    // API調査セクション
    const apiSection = document.createElement('div');
    apiSection.className = 'debug-section';
    const apiTitle = document.createElement('h3');
    apiTitle.textContent = '🔍 API調査ツール';
    const apiDescription = document.createElement('p');
    apiDescription.textContent = 'チケット情報APIの動作確認を行います。';
    // APIボタングループ
    const apiButtonGroup = document.createElement('div');
    apiButtonGroup.className = 'debug-api-button-group';
    apiButtonGroup.style.display = 'flex';
    apiButtonGroup.style.flexDirection = 'column';
    apiButtonGroup.style.gap = '8px';
    // 1. チケットIDすべての取得
    const getAllTicketsButton = document.createElement('button');
    getAllTicketsButton.className = 'debug-api-button';
    getAllTicketsButton.textContent = '📋 チケットIDすべての取得';
    getAllTicketsButton.style.backgroundColor = '#007bff';
    getAllTicketsButton.style.color = 'white';
    getAllTicketsButton.style.padding = '8px 12px';
    getAllTicketsButton.style.border = 'none';
    getAllTicketsButton.style.borderRadius = '4px';
    getAllTicketsButton.style.cursor = 'pointer';
    // 2. 入場予約済みの来場日時とチケットIDの一覧
    const getReservationsButton = document.createElement('button');
    getReservationsButton.className = 'debug-api-button';
    getReservationsButton.textContent = '🎫 入場予約済み一覧';
    getReservationsButton.style.backgroundColor = '#28a745';
    getReservationsButton.style.color = 'white';
    getReservationsButton.style.padding = '8px 12px';
    getReservationsButton.style.border = 'none';
    getReservationsButton.style.borderRadius = '4px';
    getReservationsButton.style.cursor = 'pointer';
    // 3. 入場予約ごとのパビリオン予約状況
    const getPavilionStatusButton = document.createElement('button');
    getPavilionStatusButton.className = 'debug-api-button';
    getPavilionStatusButton.textContent = '🏛️ パビリオン予約状況';
    getPavilionStatusButton.style.backgroundColor = '#ffc107';
    getPavilionStatusButton.style.color = 'black';
    getPavilionStatusButton.style.padding = '8px 12px';
    getPavilionStatusButton.style.border = 'none';
    getPavilionStatusButton.style.borderRadius = '4px';
    getPavilionStatusButton.style.cursor = 'pointer';
    apiButtonGroup.appendChild(getAllTicketsButton);
    apiButtonGroup.appendChild(getReservationsButton);
    apiButtonGroup.appendChild(getPavilionStatusButton);
    // API結果表示エリア
    const apiResultsArea = document.createElement('div');
    apiResultsArea.className = 'debug-api-results-area';
    const apiResultsTitle = document.createElement('h4');
    apiResultsTitle.textContent = '📊 API調査結果';
    const apiResultsDisplay = document.createElement('pre');
    apiResultsDisplay.className = 'debug-api-results-display';
    apiResultsDisplay.style.backgroundColor = '#f8f9fa';
    apiResultsDisplay.style.padding = '12px';
    apiResultsDisplay.style.borderRadius = '4px';
    apiResultsDisplay.style.fontSize = '12px';
    apiResultsDisplay.style.maxHeight = '300px';
    apiResultsDisplay.style.overflowY = 'auto';
    apiResultsDisplay.textContent = 'API調査結果がここに表示されます...';
    apiResultsArea.appendChild(apiResultsTitle);
    apiResultsArea.appendChild(apiResultsDisplay);
    // セクション構築
    apiSection.appendChild(apiTitle);
    apiSection.appendChild(apiDescription);
    apiSection.appendChild(apiButtonGroup);
    apiSection.appendChild(apiResultsArea);
    ...
    searchButton.addEventListener('click', async () => {
        ...
        console.log('🔍 API検索実行:', query);
        searchButton.disabled = true;
        searchButton.textContent = '検索中...';
        resultsDisplay.textContent = '検索実行中...';
        try {
            // 直接API検索を実行
            const searchParams = getCurrentPageParams();
            console.log('🔍 現在のページパラメータ:', searchParams);
            // channelパラメータは数値を使用（URLのlotteryパラメータから取得）
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            const errorStack = error instanceof Error ? error.stack || 'スタックトレースなし' : 'スタックトレースなし';
    ...
    schedulesButton.addEventListener('click', async () => {
        const pavilionCode = pavilionInput.value.trim();
        if (!pavilionCode) {
            alert('パビリオンコードを入力してください');
            return;
        }
        console.log('📅 全時間帯取得実行:', pavilionCode);
        schedulesButton.disabled = true;
        schedulesButton.textContent = '取得中...';
        resultsDisplay.textContent = '全時間帯情報を取得中...';
        try {
            // パラメータ設定
            const searchParams = getCurrentPageParams();
            const ticketIdsParam = searchParams.ticketIds.map((id) => `ticket_ids[]=${id}`).join('&');
            ...
            const entranceDateParam = `&entrance_date=${entranceDate}`;
            const lotteryParam = new URLSearchParams(window.location.search).get('lottery') || '1';
            const apiUrl = `/api/d/events/${pavilionCode}?${ticketIdsParam}${entranceDateParam}&channel=${lotteryParam}`;
            console.log('📡 API URL:', apiUrl);
            const response = await fetch(apiUrl, {
                method: 'GET',
                headers: {
            ...
            const data = await response.json();
            // 時間帯情報の整理
            if (data.event_schedules && Array.isArray(data.event_schedules)) {
                const schedulesInfo = {
                    pavilion_code: pavilionCode,
                    entrance_date: entranceDate,
                    total_schedules: data.event_schedules.length,
                    schedules: data.event_schedules.map((schedule) => ({
                        start_time: schedule.start_time,
                        schedule_name: schedule.schedule_name,
                        time_status: schedule.time_status,
                        unavailable_reason: schedule.unavailable_reason || null
                    }))
                };
                console.log('✅ 全時間帯取得成功:', schedulesInfo);
                resultsDisplay.textContent = JSON.stringify(schedulesInfo, null, 2);
            }
            else {
                resultsDisplay.textContent = `パビリオン情報取得成功:\n${JSON.stringify(data, null, 2)}`;
            }
        }
        catch (error) {
            console.error('❌ 全時間帯取得エラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            resultsDisplay.textContent = `エラー: ${errorMessage}`;
        }
        finally {
            schedulesButton.disabled = false;
            schedulesButton.textContent = '全時間帯取得';
        }
    });
    // 時間帯予約ボタンのイベントハンドラ
    reserveButton.addEventListener('click', async () => {
        ...
        if (!timeSlot) {
            alert('時間帯を入力してください（例: 1400）');
            return;
        }
        // パラメータ事前チェック
        const searchParams = getCurrentPageParams();
        console.log('🔍 取得されたページパラメータ:', searchParams);
        if (!searchParams.ticketIds || searchParams.ticketIds.length === 0) {
            alert('ticket_idsが取得できません。URLにid=パラメータが必要です。');
            return;
        }
        console.log('🎫 時間帯予約実行:', { pavilionCode, timeSlot });
        reserveButton.disabled = true;
        reserveButton.textContent = '予約中...';
        resultsDisplay.textContent = '予約実行中...';
        try {
            // パラメータ設定
            const searchParams = getCurrentPageParams();
            const entranceDate = searchParams.entranceDate || '20250826';
            const lotteryParam = new URLSearchParams(window.location.search).get('lottery') || '1';
            // registered_channelの設定（公式サイトに合わせて修正）
            const getRegisteredChannel = (channel) => {
                // 公式サイトの成功例では lottery値をそのまま使用
                console.log('🔍 lottery channel値:', channel);
                console.log('🔍 registered_channel計算結果:', channel);
                return channel; // lottery値をそのまま使用
            };
            const reservationData = {
                ticket_ids: searchParams.ticketIds,
                entrance_date: entranceDate,
                start_time: timeSlot,
                event_code: pavilionCode,
                registered_channel: getRegisteredChannel(lotteryParam)
            };
            console.log('📡 予約データ:', reservationData);
            console.log('📡 予約データ詳細:');
            console.log('  ticket_ids:', reservationData.ticket_ids);
            console.log('  entrance_date:', reservationData.entrance_date);
            console.log('  start_time:', reservationData.start_time);
            console.log('  event_code:', reservationData.event_code);
            console.log('  registered_channel:', reservationData.registered_channel);
            console.log('  JSON文字列:', JSON.stringify(reservationData));
            // CSRFトークンを取得（metaタグまたはクッキーから）
            const getCsrfToken = () => {
                // Next.jsのCSRFトークンをmetaタグから取得
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    console.error('❌ エラーレスポンス詳細:', errorBody);
                }
                catch (e) {
                    console.error('❌ エラーレスポンス読み取り失敗:', e);
                }
                throw new Error(`予約API Error: ${response.status} ${response.statusText}\nレスポンス: ${errorBody}`);
            }
            const data = await response.json();
            console.log('✅ 予約成功:', data);
            resultsDisplay.textContent = `予約成功:\n${JSON.stringify(data, null, 2)}`;
        }
        catch (error) {
            console.error('❌ 予約エラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            resultsDisplay.textContent = `予約エラー: ${errorMessage}`;
        }
        finally {
    ...
    navButton.addEventListener('click', async () => {
        const pavilionCode = navInput.value.trim();
        console.log('🚀 画面遷移テスト実行:', pavilionCode);
        navButton.disabled = true;
        navButton.textContent = '遷移中...';
        try {
            // 現在のページの検索フィールドからkeywordを取得
            ...
            const getCurrentPageParams = () => {
                ...
                return {
                    ...
                    screenId: urlParams.get('screen_id') || '108',
                    priority: urlParams.get('priority'),
                    keyword: getCurrentSearchQuery(),
                    reserveId: urlParams.get('reserve_id') || ''
                };
            };
            // 公式サイトのURL生成ロジックを再現
            const buildEventTimeUrl = (pavilionCode) => {
                const params = getCurrentPageParams();
                const t = ''; // type=change フラグ
                const _ = params.priority ? `&priority=${params.priority}` : '';
                const n = `&event_type=${params.eventType}`;
                const s = params.entranceDate ? `&entrance_date=${params.entranceDate}` : '';
                const a = `?id=${params.ticketIds.join(',')}&event_id=${pavilionCode}&screen_id=${params.screenId}${_}&lottery=${params.lottery}&keyword=${params.keyword}${n}&reserve_id=${params.reserveId}${s}`;
                return `/event_time${a}${t}`;
            };
            console.log('🔍 Step 1: Router存在確認開始');
            const routerInfo = RouterUtils.check();
            console.log('🔍 Step 2: Router確認結果:', routerInfo ? 'Router発見' : 'Router未発見');
            console.log('🔍 Step 3: URL構築開始');
            const url = buildEventTimeUrl(pavilionCode);
            console.log('🔍 Step 4: 構築されたURL:', url);
            if (routerInfo?.exists) {
                console.log('🔍 Step 5: Router.push()実行開始');
                const success = RouterUtils.push(url);
                if (success) {
                    console.log('✅ Step 6: Router.push()実行成功');
                }
                else {
                    console.error('❌ Step 6: Router.push()実行失敗');
                    throw new Error('Router.push()実行に失敗しました');
                }
            }
            else {
                console.error('❌ Next.js Router が見つかりません');
                throw new Error('Router取得に失敗しました');
            }
            // ダイアログを閉じる
            setTimeout(() => {
                dialogOverlay.remove();
            ...
            console.error('❌ 画面遷移テストエラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            alert(`画面遷移テストエラー: ${errorMessage}`);
        }
        finally {
    ...
    // API調査ボタンのイベントハンドラ
    // 1. チケットIDすべての取得
    getAllTicketsButton.addEventListener('click', async () => {
        console.log('🔍 チケットID全取得API実行');
        getAllTicketsButton.disabled = true;
        getAllTicketsButton.textContent = '取得中...';
        apiResultsDisplay.textContent = '実行中...';
        try {
            ...
            const data = await response.json();
            console.log('📋 チケット一覧データ:', data);
            // チケット情報をわかりやすく整形
            let displayText = '=== チケットIDすべての取得結果 ===\n\n';
            if (data.list && Array.isArray(data.list)) {
                displayText += `総チケット数: ${data.count || data.list.length}件\n\n`;
                data.list.forEach((ticket, index) => {
                    displayText += `【チケット${index + 1}】\n`;
                    displayText += `  チケットID: ${ticket.ticket_id || 'N/A'}\n`;
                    displayText += `  簡易ID: ${ticket.simple_ticket_id || 'N/A'}\n`;
                    displayText += `  チケット名: ${ticket.item_name || 'N/A'}\n`;
                    displayText += `  入場予約数: ${ticket.schedules ? ticket.schedules.length : 0}件\n`;
                    if (ticket.schedules && ticket.schedules.length > 0) {
                        ticket.schedules.forEach((schedule, schedIndex) => {
                            displayText += `    [予約${schedIndex + 1}] ${schedule.entrance_date} (状態: ${schedule.use_state})\n`;
                        });
                    }
                    displayText += '\n';
                });
                ...
                displayText += 'チケットデータが見つかりませんでした。\n';
            }
            displayText += '\n=== 生JSONデータ ===\n';
            displayText += JSON.stringify(data, null, 2);
            apiResultsDisplay.textContent = displayText;
        }
        catch (error) {
            console.error('❌ チケット取得API エラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            apiResultsDisplay.textContent = `エラー: ${errorMessage}`;
        }
        finally {
            getAllTicketsButton.disabled = false;
            getAllTicketsButton.textContent = '📋 チケットIDすべての取得';
        }
    });
    // 2. 入場予約済みの来場日時とチケットIDの一覧
    getReservationsButton.addEventListener('click', async () => {
        console.log('🎫 入場予約済み一覧API実行');
        getReservationsButton.disabled = true;
        getReservationsButton.textContent = '取得中...';
        apiResultsDisplay.textContent = '実行中...';
        try {
            const response = await fetch('/api/d/my/tickets/?count=1', {
                method: 'GET',
            ...
            const data = await response.json();
            console.log('🎫 入場予約済み一覧データ:', data);
            // 入場予約情報を抽出・整形
            let displayText = '=== 入場予約済み来場日時・チケットID一覧 ===\n\n';
            let totalReservations = 0;
            if (data.list && Array.isArray(data.list)) {
                data.list.forEach((ticket) => {
                    if (ticket.schedules && ticket.schedules.length > 0) {
                        displayText += `【チケット: ${ticket.simple_ticket_id}】\n`;
                        ticket.schedules.forEach((schedule, schedIndex) => {
                            displayText += `  予約${schedIndex + 1}: ${schedule.entrance_date} - `;
                            displayText += `入場予約ID: ${schedule.user_visiting_reservation_id || 'N/A'} - `;
                            displayText += `状態: ${schedule.use_state || 'N/A'}\n`;
                            if (schedule.group_ticket_qr_divi) {
                                displayText += `    QR分割: ${schedule.group_ticket_qr_divi}\n`;
                            }
                            totalReservations++;
                        });
                        displayText += '\n';
                ...
                displayText += `総入場予約数: ${totalReservations}件\n\n`;
            }
            else {
                displayText += '入場予約データが見つかりませんでした。\n\n';
            }
            displayText += '=== 生JSONデータ ===\n';
            displayText += JSON.stringify(data, null, 2);
            apiResultsDisplay.textContent = displayText;
        }
        catch (error) {
            console.error('❌ 入場予約取得API エラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            apiResultsDisplay.textContent = `エラー: ${errorMessage}`;
        }
        finally {
            getReservationsButton.disabled = false;
            getReservationsButton.textContent = '🎫 入場予約済み一覧';
        }
    });
    // 3. 入場予約ごとのパビリオン予約状況
    getPavilionStatusButton.addEventListener('click', async () => {
        console.log('🏛️ パビリオン予約状況API実行');
        getPavilionStatusButton.disabled = true;
        getPavilionStatusButton.textContent = '取得中...';
        apiResultsDisplay.textContent = '実行中...';
        try {
            // まずチケット情報を取得して、入場日を特定
            const ticketsResponse = await fetch('/api/d/my/tickets/?count=1', {
                method: 'GET',
                headers: {
                    'Accept-Language': 'ja,en-US;q=0.9,en;q=0.8,zh-TW;q=0.7,zh;q=0.6',
                    'X-Api-Lang': 'ja'
            ...
            if (!ticketsResponse.ok) {
                throw new Error(`Tickets API Error: ${ticketsResponse.status} ${ticketsResponse.statusText}`);
            }
            const ticketsData = await ticketsResponse.json();
            console.log('🏛️ チケットデータ取得:', ticketsData);
            let displayText = '=== 入場予約ごとのパビリオン予約状況 ===\n\n';
            if (ticketsData.list && Array.isArray(ticketsData.list) && ticketsData.list.length > 0) {
                // 各チケットの入場予約に対して抽選カレンダー情報を取得
                for (const ticket of ticketsData.list) {
                    displayText += `【チケット: ${ticket.simple_ticket_id}】\n`;
                    if (ticket.schedules && ticket.schedules.length > 0) {
                        // 状態1でない入場予約のみを処理
                        const unusedSchedules = ticket.schedules.filter((schedule) => schedule.use_state !== 1);
                        if (unusedSchedules.length > 0) {
                            displayText += `  未使用入場予約: ${unusedSchedules.length}件\n`;
                            for (const schedule of unusedSchedules) {
                                const entranceDate = schedule.entrance_date;
                                displayText += `\n  【入場日: ${entranceDate} - ${schedule.schedule_name} - 状態: ${schedule.use_state}】\n`;
                                // このチケットでの予約タイプ取得状況（なし/提出/あり）
                                displayText += `    このチケットの予約取得状況:\n`;
                                // 2ヶ月前抽選の状況判定
                                let monthStatus = 'なし';
                                if (schedule.lotteries?.month?.length > 0) {
                                    // 抽選結果があるかチェック
                                    const monthLottery = schedule.lotteries.month[0];
                                    monthStatus = monthLottery.state === 1 ? 'あり' :
                                        monthLottery.state === 0 ? '提出' : 'なし';
                                }
                                else if (schedule.month_lottery) {
                                    monthStatus = 'あり';
                                }
                                // 7日前抽選の状況判定  
                                let dayStatus = 'なし';
                                if (schedule.lotteries?.day?.length > 0) {
                                    const dayLottery = schedule.lotteries.day[0];
                                    dayStatus = dayLottery.state === 1 ? 'あり' :
                                        dayLottery.state === 0 ? '提出' : 'なし';
                                }
                                else if (schedule.day_lottery) {
                                    dayStatus = 'あり';
                                }
                                displayText += `      2ヶ月前抽選: ${monthStatus}\n`;
                                displayText += `      7日前抽選: ${dayStatus}\n`;
                                displayText += `      空き枠予約: ${schedule.empty_frame ? 'あり' : 'なし'}\n`;
                                displayText += `      当日予約: ${schedule.on_the_day ? 'あり' : 'なし'}\n`;
                                // この入場日に予約済みのパビリオン情報
                                if (ticket.event_schedules) {
                                    const eventSchedules = ticket.event_schedules.filter((es) => es.entrance_date === entranceDate);
                                    if (eventSchedules.length > 0) {
                                        displayText += `    パビリオン予約状況:\n`;
                                        eventSchedules.forEach((es, esIndex) => {
                                            const useStateText = es.use_state === 0 ? '未使用' :
                                                es.use_state === 1 ? '使用済み' :
                                                    `状態${es.use_state}`;
                                            displayText += `      [${esIndex + 1}] ${es.event_name}\n`;
                                            displayText += `          時間: ${es.schedule_name} (${es.start_time}-${es.end_time})\n`;
                                            displayText += `          状態: ${useStateText}\n`;
                                            displayText += `          チャンネル: ${es.registered_channel}\n`;
                                            displayText += `          プログラム状態: ${es.program_ticket_state}\n`;
                                            if (es.admission_time) {
                                                displayText += `          入場時刻: ${es.admission_time}\n`;
                                            }
                                        });
                                    }
                                    else {
                                        displayText += `    パビリオン予約: なし\n`;
                                    }
                                }
                                // 抽選カレンダー情報も取得（各入場日ごと）
                                try {
                                    const calendarResponse = await fetch(`/api/d/lottery_calendars?entrance_date=${entranceDate}`, {
                                        method: 'GET',
                                        headers: {
                                            'Accept-Language': 'ja,en-US;q=0.9,en;q=0.8,zh-TW;q=0.7,zh;q=0.6',
                                            'X-Api-Lang': 'ja'
                                    ...
                                    if (calendarResponse.ok) {
                                        ...
                                        // データ構造を確認 - calendarData.data または calendarData 直接
                                        const lotteryData = calendarData.data || calendarData;
                                        if (lotteryData && typeof lotteryData === 'object') {
                                            ...
                                            const twoMonthsOpen = checkPeriod(lotteryData.two_months_ago_lottery);
                                            const sevenDaysOpen = checkPeriod(lotteryData.seven_days_ago_lottery);
                                            const emptyFrameOpen = checkPeriod(lotteryData.empty_frame_reservation);
                                            const onTheDayOpen = checkPeriod(lotteryData.on_the_day_reservation);
                                            displayText += `    システム全体の予約枠開放状況:\n`;
                                            displayText += `      2ヶ月前抽選: ${twoMonthsOpen ? 'OPEN' : 'CLOSED'}\n`;
                                            displayText += `      7日前抽選: ${sevenDaysOpen ? 'OPEN' : 'CLOSED'}\n`;
                                            displayText += `      空き枠予約: ${emptyFrameOpen ? 'OPEN' : 'CLOSED'}\n`;
                                            displayText += `      当日予約: ${onTheDayOpen ? 'OPEN' : 'CLOSED'}\n`;
                                        }
                                        else {
                                            console.warn(`⚠️ ${entranceDate}: 抽選データが見つかりません`, lotteryData);
                                            displayText += `    システム全体の予約枠開放状況: 構造不明\n`;
                                        }
                                    }
                                    else {
                                        console.error(`❌ ${entranceDate}: 抽選カレンダーAPI失敗 ${calendarResponse.status}`);
                                        displayText += `    システム全体の予約枠開放状況: 取得エラー (${calendarResponse.status})\n`;
                                    }
                                }
                                catch (calendarError) {
                                    displayText += `    抽選カレンダー取得エラー: ${calendarError}\n`;
                                }
                                // 抽選状況詳細（最後に表示）
                                if (schedule.lotteries) {
                                    if (schedule.lotteries.month && schedule.lotteries.month.length > 0) {
                                        displayText += `    2ヶ月前抽選詳細:\n`;
                                        schedule.lotteries.month.forEach((lottery, index) => {
                                            displayText += `      [${index + 1}] 状態: ${lottery.state}, 日付: ${lottery.entrance_date}\n`;
                                        });
                                    }
                                    if (schedule.lotteries.day && schedule.lotteries.day.length > 0) {
                                        displayText += `    7日前抽選詳細:\n`;
                                        schedule.lotteries.day.forEach((lottery, index) => {
                                            displayText += `      [${index + 1}] 状態: ${lottery.state}, 日付: ${lottery.entrance_date}\n`;
                                        });
                                    }
                                }
            ...
            displayText += '=== チケット生JSONデータ ===\n';
            displayText += JSON.stringify(ticketsData, null, 2);
            apiResultsDisplay.textContent = displayText;
        }
        catch (error) {
            console.error('❌ パビリオン予約状況API エラー:', error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            apiResultsDisplay.textContent = `エラー: ${errorMessage}`;
        }
        finally {
            getPavilionStatusButton.disabled = false;
            getPavilionStatusButton.textContent = '🏛️ パビリオン予約状況';
        }
    });
    console.log('✅ デバッグダイアログ表示完了');
};
// エクスポート
...
class TicketManager {
    ...
    extractEntranceDates(ticket) {
        const dates = [];
        try {
            // チケット内のschedulesから入場日を抽出（デバッグダイアログで確認済み構造）
            if (ticket.schedules && Array.isArray(ticket.schedules)) {
                for (const schedule of ticket.schedules) {
                    if (schedule.entrance_date && !dates.includes(schedule.entrance_date)) {
    ...
    extractEntranceReservations(ticket) {
        const reservations = [];
        try {
            // schedulesから入場予約情報を抽出（実際のAPI構造）
            if (ticket.schedules && Array.isArray(ticket.schedules)) {
                for (const schedule of ticket.schedules) {
                    if (schedule.entrance_date && schedule.schedule_name) {
                        reservations.push({
                            date: schedule.entrance_date,
                            time: schedule.schedule_name,
                            location: schedule.gate_type === 1 ? 'east' : 'west',
                            status: schedule.use_state === 0 ? 'confirmed' : 'pending'
                        });
                    }
                }
            }
     ...
     * チケット種別から予約種類を判定
     */
    determineReservationType(_ticketType) {
        // 実装は既存のビジネスロジックに基づく
        // 現在は1日券として扱う（今後拡張）
        return {
            type: '1日券',
...
class PavilionManager {
     ...
     * 検索パラメータを構築
     */
    buildSearchParams(query, ticketIds, entranceDate) {
        return {
     ...
     * パビリオン予約実行
     */
    async makeReservation(pavilionId, timeSlot, selectedTickets) {
     ...
     * 予約API実行
     */
    async executeReservationAPI(request) {
        try {
            const response = await fetch('/api/d/user_event_reservations', {
                method: 'POST',
                headers: {
...
class MainDialogFabImpl {
    ...
    addYTFabButton() {
        ...
        `;
        // ログイン状態をチェックして初期状態を設定
        this.updateYTButtonState();
        // クリックイベントを設定
    ...
    isUserLoggedIn() {
        ...
        if (userInfo && userInfo.textContent && userInfo.textContent.trim() !== '') {
            return true;
        }
        // 方法3: localStorageのセッション情報
        try {
            const sessionData = localStorage.getItem('expo_session') ||
            ...
            console.warn('⚠️ localStorage確認エラー:', error);
        }
        // 方法4: Cookieでセッション確認
        const hasSessionCookie = document.cookie.includes('session') ||
            document.cookie.includes('auth') ||
            document.cookie.includes('ticket');
    ...
    updateYTButtonState() {
        ...
        if (isLoggedIn) {
            this.ytFabButton.disabled = false;
            this.ytFabButton.classList.remove('disabled');
            this.ytFabButton.title = 'メインダイアログを開く';
            this.ytFabButton.style.opacity = '1';
        }
        else {
            this.ytFabButton.disabled = true;
            this.ytFabButton.classList.add('disabled');
            this.ytFabButton.title = 'ログインが必要です';
            this.ytFabButton.style.opacity = '0.5';
        }
    }
    /**
     * ログイン状態の監視を開始
    ...
    startLoginStateMonitoring() {
        ...
        // 注意: MutationObserverは無限ループを引き起こすため無効化
        // 必要に応じて手動でログイン状態をチェック
    }
    /**
     * ログイン必須メッセージを表示
     */
    showLoginRequiredMessage() {
        // 既存のメッセージを削除
        const existingMessage = document.getElementById('ytomo-login-required-message');
        if (existingMessage) {
            existingMessage.remove();
        }
        // メッセージを作成
        const message = document.createElement('div');
        message.id = 'ytomo-login-required-message';
        message.className = 'ytomo-login-required-message';
        message.innerHTML = `
            <div class="ytomo-message-content">
                <span class="ytomo-message-icon">🔐</span>
                <span class="ytomo-message-text">ログインが必要です</span>
            </div>
        `;
        // スタイルを適用
        ...
        `;
        // アニメーション用のキーフレームを追加
        if (!document.getElementById('ytomo-login-message-styles')) {
            const style = document.createElement('style');
            style.id = 'ytomo-login-message-styles';
            style.textContent = `
                @keyframes slideIn {
                    to {
                        opacity: 1;
    ...
    async initializeTicketTab() {
        ...
        try {
            ...
            // チケットタブUIを構築
            this.buildTicketTabUI(ticketTab, tickets, availableDates);
            // タブカウント更新
     ...
     * チケットタブUIを構築
     */
    buildTicketTabUI(container, tickets, availableDates) {
        container.innerHTML = `
     ...
     * 入場日時ボタンを構築
     */
    buildEntranceDateButtons(dates) {
        return dates.map(date => `
    ...
    async handleMakeReservation() {
        ...
        try {
            ...
            const result = await this.pavilionManager.makeReservation(pavilionId, timeSlot, selectedTickets);
            if (result.success) {
                this.showReservationResult(`🎉 予約成功！${result.details?.pavilionName} ${result.details?.timeSlot}～`, 'success');
                // UI更新
     ...
     * パビリオンタブタイプを更新
     */
    updatePavilionTabType(reservationType) {
...
class AutomationEngine {
    ...
    async checkRedirectAbnormality() {
        ...
        const expectedUrl = reservationData.reservationUrl || '';
        this.log(`🔍 最終検証:`);
        this.log(`   タイトル - 期待="${expectedTitle}", 実際="${finalTitle}"`);
        this.log(`   URL - 期待="${expectedUrl}", 実際="${finalUrl}"`);
        // タイトルまたはURLが期待値と異なる場合は異常リダイレクト
        const titleMismatch = finalTitle !== expectedTitle;
        const urlMismatch = this.checkUrlMismatch(finalUrl, expectedUrl, reservationData);
        if ((titleMismatch || urlMismatch) && consecutiveStableCount >= 4) {
            if (titleMismatch && urlMismatch) {
                this.log('❌ リダイレクト異常を検知: タイトルとURLが両方一致せず、ページが安定している');
            }
            else if (titleMismatch) {
                this.log('❌ リダイレクト異常を検知: タイトルが一致せず、ページが安定している');
            }
            else {
                this.log('❌ リダイレクト異常を検知: URLが予約ページではなく、ページが安定している');
            }
            // 本来遷移するはずだったURLを表示
            const currentUrl = window.location.href;
            const expectedUrl = reservationData.reservationUrl || '予約URLが保存されていません';
            this.log(`🔗 異常リダイレクト詳細:`);
            this.log(`   現在のURL: ${currentUrl}`);
            this.log(`   本来の遷移先URL: ${expectedUrl}`);
            // 予約データを失敗状態に更新
            const { PavilionReservationCache } = await Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 619));
    ...
    /**
     * URLのミスマッチを判定（USPパラメータの追加や順序変更に対応）
     * @param actualUrl 実際のURL
     * @param expectedUrl 期待されるURL
     * @param reservationData 予約データ
     * @returns URLが異常な場合true、正常な場合false
     */
    checkUrlMismatch(actualUrl, expectedUrl, _reservationData) {
        if (!expectedUrl) {
            this.log('⚠️ 期待URLが空のためURL検証をスキップ');
            return false;
        }
        try {
            ...
            // ホスト・パス・基本構造の比較
            if (actualUrlObj.origin !== expectedUrlObj.origin ||
                actualUrlObj.pathname !== expectedUrlObj.pathname) {
                this.log(`❌ URL基本構造ミスマッチ: オリジン・パスが異なる`);
                return true;
            }
            // 必須パラメータの存在確認
            const requiredParams = ['id', 'event_id', 'screen_id', 'lottery', 'entrance_date'];
            const actualParams = actualUrlObj.searchParams;
            const expectedParams = expectedUrlObj.searchParams;
            for (const param of requiredParams) {
                const actualValue = actualParams.get(param);
                const expectedValue = expectedParams.get(param);
                if (!actualValue || !expectedValue) {
                    this.log(`❌ 必須パラメータ不足: ${param}`);
                    return true;
                }
                if (actualValue !== expectedValue) {
                    this.log(`❌ 必須パラメータ値ミスマッチ: ${param} (期待=${expectedValue}, 実際=${actualValue})`);
                    return true;
                }
            }
            // 追加パラメータのログ出力（エラーではない）
            const expectedParamKeys = Array.from(expectedParams.keys());
            const actualParamKeys = Array.from(actualParams.keys());
            const additionalParams = actualParamKeys.filter(key => !expectedParamKeys.includes(key));
            if (additionalParams.length > 0) {
                this.log(`ℹ️ 追加パラメータを検出（許可）: ${additionalParams.join(', ')}`);
            }
            this.log('✅ URL検証OK: 構造とパラメータが正常');
            return false;
        }
        catch (error) {
            this.log(`❌ URL解析エラー: ${error}`);
            return true;
        }
    }
}
