...
const cacheManager = {
    ...
    saveTargetSlots() {
        try {
            ...
            // 現在選択されているカレンダー日付を取得
            const selectedCalendarDate = getCurrentSelectedCalendarDate();
...
function handleMonitorButtonClick(slotInfo, buttonElement) {
    ...
    if (timeSlotState.isMonitoring) {
        ...
        // ユーザーにフィードバックを提供
        showStatus('監視実行中のため操作できません', 'orange');
        return;
    }
...
function shouldUpdateMonitorButtons() {
    ...
    // 満員時間帯数と既存ボタン数が大きく異なる場合は更新
    const fullSlotCount = analysis.full.length;
    const existingButtonCount = existingButtons.length;
    
    // 監視対象のボタンを除外した純粋な満員ボタン数をカウント
    const nonMonitoringButtons = Array.from(existingButtons).filter(btn => {
        const span = btn.querySelector('span');
        return span && !span.innerText.startsWith('監視');
    });
    
    const hasSignificantChange = Math.abs(fullSlotCount - nonMonitoringButtons.length) > 0;
    
    console.log(`🔍 更新判定: 満員=${fullSlotCount}, 既存ボタン=${existingButtonCount}, 非監視ボタン=${nonMonitoringButtons.length}, 更新必要=${hasSignificantChange}`);
...
function restoreSelectionAfterUpdate() {
    ...
    const targets = multiTargetManager.getTargets();
    console.log(`🔄 選択状態を復元中: ${targets.length}個の監視対象`);
    
    targets.forEach(target => {
        // 対応するボタンを検索
        const allButtons = document.querySelectorAll('.monitor-btn');
        allButtons.forEach(button => {
            const timeText = button.getAttribute('data-target-time');
            const tdElement = button.closest('td[data-gray-out]');
            const tdSelector = tdElement ? generateUniqueTdSelector(tdElement) : '';
            
            if (timeText === target.timeText && tdSelector === target.tdSelector) {
                // ボタンの表示を復元
                const span = button.querySelector('span');
                if (span) {
                    const targets = multiTargetManager.getTargets();
                    ...
                    if (targetIndex >= 0) {
                        ...
                        span.innerText = `監視${priority}`;
                        button.style.background = 'rgb(0, 104, 33)';
                        console.log(`✅ 選択状態を復元: ${target.timeText} (優先順位: ${priority})`);
                    }
                }
            }
        });
...
async function startSlotMonitoring() {
    ...
    updateMainButtonDisplay(); // 即座にボタン表示を更新
...
async function checkSlotAvailabilityAndReload() {
    ...
    if (currentSlot && currentSlot.status === 'available') {
        ...
        // ボタン表示を更新（見つかりましたモード）
        updateMainButtonDisplay('found-available');
        
        // 自動選択
        await selectTimeSlotAndStartReservation(currentSlot);
        return;
    }
    ...
    // カウントダウン開始（即座にUI更新）
    startReloadCountdown(displaySeconds);
    
    // リロードタイマーを保存（中断時に停止するため）
    reloadCountdownState.reloadTimer = setTimeout(() => {
...
// 入場予約ページの初期化判定
function judge_entrance_init() {
    // 基本的なページ要素の存在確認
    const mainElement = document.querySelector('#__next > div > div > main');
    const headerElement = document.querySelector('header');
    
    return !!(mainElement && headerElement);
}

// 入場予約ページ初期化の実行
function init_entrance_page() {
    console.log('入場予約ページを初期化中...');
    ...
    setTimeout(() => {
        ...
        console.log('✅ 入場予約ページの初期化完了');
    }, 1000);
}

// パビリオン予約ページの初期化判定
function judge_init() {
    const inputBox = document.querySelector('input[type="text"][name="keyword"]');
    const selBoxes = document.querySelectorAll('select');
    return inputBox && (selBoxes.length >= 3);
}
...
function init_page() {
    ...
    input_div.classList.add("div-flex");
    
    const input_search = document.createElement("input");
    input_search.type = "text";
    input_search.name = "keyword_ytomo";
    input_search.placeholder = "ytomo検索 ";
    input_search.className = "ext-tomo search";
    
    const apply_filter = () => {
        const val_search = input_search.value;
        const { include, exclude } = prepare_filter(val_search);
        
        const eles_pavilion = Array.from(document.querySelectorAll(
            "div.view-card-list > div > div > div:nth-child(2)"
        ));
        
        let count_show = 0;
        eles_pavilion.forEach(element => {
            const text_content = element.textContent || "";
            
            const shouldShow = include.test(text_content) && 
                             (!exclude || !exclude.some(pattern => pattern.test(text_content)));
            
            const parent_div = element.closest("div.view-card-list > div");
            if (parent_div) {
                if (shouldShow) {
                    parent_div.classList.remove("safe-none", "ytomo-none", "filter-none");
                    count_show++;
                } else {
                    parent_div.classList.add("ytomo-none");
                }
            }
        });
        
        console.log(`表示パビリオン数: ${count_show}`);
    };
    
    input_search.addEventListener("input", apply_filter);
    
    input_div.appendChild(input_search);
    
    const target_parent = document.querySelector(
        "#__next > div > div > main > div > div"
    );
    const target_child = document.querySelector(
        "#__next > div > div > main > div > div > div.style_main__filter__CNNHy"
    );
    
    if (target_parent && target_child) {
        target_parent.insertBefore(input_div, target_child);
    }
...
// テスト環境でのモジュールエクスポート（Node.js環境対応）
