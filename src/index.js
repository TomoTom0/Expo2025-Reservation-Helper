...
class TicketManager {
            ...
            if (data.list && Array.isArray(data.list)) {
                for (const ticket of data.list) {
                    ...
                    if (ticketData.schedules && ticketData.schedules.length > 0) {
                        ticketData.schedules.forEach((schedule, i) => {
     ...
     * 外部チケットの詳細データを取得
     */
    async loadExternalTicketData(ticketId, label, channel) {
        try {
     ...
     * 外部チケットIDを追加
     */
    async addExternalTicket(ticketId, label, channel) {
     ...
     * チケットIDの妥当性を検証
     */
    async validateTicketId(ticketId, channel) {
        // チケットIDの形式チェック
     ...
     * 外部チケットをキャッシュに保存
     */
    async saveExternalTicketToCache(ticketId, label, channel) {
        try {
            // 監視キャッシュに保存
            const monitoringCache = localStorage.getItem('expo_monitoring_cache') || '{}';
...
class MainDialogFabImpl {
    ...
    async handleAddTicket() {
        ...
        const channelSelect = this.mainDialogContainer?.querySelector('#channel-select');
        if (!ticketIdInput)
            return;
        ...
        try {
            ...
            console.log(`🎫 追加されたチケットID: ${ticketId} (channel: ${channel}) が含まれているか:`, allTickets.some(t => t.ticket_id === ticketId));
            // 成功時はタブを再初期化
...
;// ./ts/modules/monitoring-scheduler.ts
/**
 * 監視スケジューラー
 * 変更容易な監視タイミング制御
 */
// デフォルト設定（毎分00,15,30,45秒）
const monitoring_scheduler_DEFAULT_CONFIG = {
    intervalType: 'fixed-seconds',
    fixedSeconds: [0, 15, 30, 45]
};
class MonitoringScheduler {
    constructor(config = monitoring_scheduler_DEFAULT_CONFIG) {
        this.callback = null;
        this.timeoutId = null;
...
;// ./ts/modules/monitoring-cache.ts
/**
 * 監視対象キャッシュ管理
 * 順序ベースの監視対象データ管理
 */
...
;// ./ts/modules/monitoring-service.ts
/**
 * 監視サービス
 * API呼び出し・空き検知・自動予約実行
 */
...
class MonitoringService {
    constructor() {
        this.scheduler = getMonitoringScheduler();
        this.isRunning = false;
    }
    /**
     * 監視を開始
    ...
    async executeReservation(target) {
        ...
        try {
            ...
            // キャッシュに保存
            pavilion_reservation_cache.PavilionReservationCache.saveReservationData(target.pavilionCode, reservationData);
            // 予約ページを開く
...
;// ./ts/modules/immediate-reservation.ts
/**
 * 即時予約機能
 * 空き時間帯での即座予約実行
 */
...
class ImmediateReservationService {
    ...
    static async executeReservation(pavilionCode, pavilionName, timeSlot, timeDisplay) {
        ...
        try {
            ...
            pavilion_reservation_cache.PavilionReservationCache.saveReservationData(pavilionCode, cacheData);
            // ページ復帰システムは無効化
            // 3. 予約ページに遷移
            // expoTable.jsを参照した正しいURL実装
            const ticketIds = new URLSearchParams(window.location.search).get('id') || '';
