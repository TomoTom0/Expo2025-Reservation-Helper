...
async function selectTimeSlotAndStartReservation(slotInfo) {
    ...
    setTimeout(async () => {
        ...
        if (config && entranceReservationHelper) {
            ...
            if (result.success && section6_cacheManager) {
                section6_cacheManager.clearTargetSlots(); // 成功時はキャッシュクリア
                section6_cacheManager.clearMonitoringFlag(); // 監視継続フラグもクリア
...
function stopSlotMonitoring() {
    ...
    if (section6_cacheManager) {
        section6_cacheManager.clearMonitoringFlag();
    }
    // リロードカウントダウンも確実に停止
    stopReloadCountdown();
...
function restoreSelectionAfterUpdate() {
    ...
    if (restoredCount === 0) {
        ...
        timeSlotState.mode = 'idle';
        if (section6_cacheManager) {
            section6_cacheManager.clearTargetSlots();
        }
    }
    updateMainButtonDisplay();
}
...
function updateMainButtonDisplay(forceMode = null) {
    ...
    if (fabButton && statusBadge) {
        ...
        if (span) {
            ...
            // デバッグ情報
            const targetTexts = section2_multiTargetManager.hasTargets() ? section2_multiTargetManager.getTargets().map(t => t.timeText).join(', ') : 'なし';
...
function getCurrentMode() {
    ...
    else if (section2_multiTargetManager.hasTargets() && timeSlotState.mode === 'selecting') {
        return 'selecting';
    }
    else {
        return 'idle';
...
async function restoreFromCache() {
    if (!section6_cacheManager)
        return;
    const cached = section6_cacheManager.loadTargetSlots();
    if (!cached)
        return;
    ...
    setTimeout(() => {
        ...
        if (restoredCount > 0) {
            ...
            // 監視継続フラグをチェックして監視を再開
            const shouldContinueMonitoring = section6_cacheManager?.getAndClearMonitoringFlag();
