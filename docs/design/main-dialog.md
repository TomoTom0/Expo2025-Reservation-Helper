...
流れを説明すると毎分35秒  │
│   から次の流れを行う。「最優先の予約実行->結果確認」と「その月の入場空き   │
│   情報取得してその日の時間帯情報取得」を並行で進める、その後その日の時間   │
│   帯情報の中で目標の時間帯に満員でない時間帯があれば優先度に従って最大2つ  │
│   の目標(実行済みの最優先は除く)に対して「予約実行->結果確認->予約実行->   │
│   結果確認」を行う。その後、次の分の目標時間まで待機する。この道中の状況   │
│   を予約実行時情報のさらに下に表示する
