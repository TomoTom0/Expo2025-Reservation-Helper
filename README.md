...
パビリオン・イベント検索画面、入場予約画面、チケット選択画面、同行者追加画面に対応しています。

- [yt Expo2025 先着予約 Helper](#yt-expo2025-先着予約-helper)
  - [Feature](#feature)
...
入場予約画面では、満員時間帯の監視と自動予約機能が利用できます。

| 画面要素 | 機能 | 説明 |
| --- | --- | --- |
| 満員時間帯の「満員/監視」ボタン | 時間帯監視 | 満員時間帯を監視対象に追加します。複数の時間帯を同時に監視できます。 |
| 右下FABの「監視予約開始」ボタン | 時間帯監視 | 30秒間隔で監視対象をチェックし、空きが出たら自動で予約処理に移行します。 |
| 右下FABの「予約開始」ボタン | 入場予約自動化 | 予約成功まで自動でリトライを繰り返します（最大100回試行）。 |

#### 時間帯監視

人気の入場券はすぐに満員になってしまいます。でもキャンセルが出て空きになることもあります。~~手動でリロードし続けるのは大変。~~

満員時間帯に表示される「満員」ボタンをクリックすると、その時間帯が監視対象に追加されます。
複数時間帯の同時監視も可能です。
「監視予約開始」を押下すると、約30秒間隔でページを自動リロードし、監視対象の空き状況を確認します。

中断ボタンを押下すると、監視を中断します。



監視対象に空きが検出されると、自動予約処理に移行します。



複数の対象が同時に空きになった場合、優先順位に沿って予約対象を決定します。
(監視1 > 監視2 > 監視3 ...)

#### 入場予約自動化

満員でない時間帯を選択して「予約開始」ボタンを押下すると、予約成功まで自動で予約リトライを継続します。
最大100回まで試行し、失敗時には適切な間隔で待機してから再試行します。


現在の試行回数がリアルタイムで表示され、処理を中断することも可能です。
