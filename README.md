...
| 処理オーバーレイの🔊/🔇ボタン | 音声通知切り替え | 予約成功時の音声通知のオン/オフを切り替えます。 |
...
予約成功時には音声通知でお知らせします（オーバーレイの🔊/🔇ボタンで切り替え可能）。
...
本拡張子専用の画面で設定の変更や高度な利用ができます。

#### 他タブ

- API利用: 本拡張子利用におけるAPIの直接利用を設定できます
  - なし(デフォルト): 本拡張子の機能を利用する際に、一切APIを直接利用しません
  - 抑制: YTomo画面以外では、一切APIを直接利用しません
  - あり: YTomo画面以外でも、APIを適度に利用します
- 入場予約実行目標秒時間: 入場予約自動化機能における目標時間を設定できます
  - デフォルト: 35秒 (毎分35秒の予約実行を目標とします)

API利用が「なし」以外の場合、追加の要素やタブが利用できます。
利用に際しては、万博IDのBANのリスクをご理解ください。
(API利用がなしなら、リスクは非常に低いと思われますが、断言はできません)
...
2. ブラウザでGitHubの本リポジトリを開き、`src/index.js`を開きます: [link](https://github.com/TomoTom0/expo2025-extension/blob/main/src/index.js)。
...
Updateは、Installの場合の「2-5」を実行してください。
既存の機能を削除せずとも更新されます。

### Chrome on Windows

1. [Releases](https://github.com/TomoTom0/Expo2025-Reservation-Helper/releases)から最新の`src.zip`をダウンロードして展開してください。

2. Chromeの拡張機能管理画面を開いてください。
   - アドレスバーに `chrome://extensions/` と入力してEnter
   - または、Chrome右上の三点メニュー → 拡張機能 → 拡張機能を管理

3. 右上の「デベロッパーモード」を有効にしてください。

4. 「パッケージ化されていない拡張機能を読み込む」をクリックし、展開した`src`フォルダを選択してください。
...
1. Installと同様にダウンロードして展開して、既存のファイルを上書きしてください。

2. Chromeの拡張機能管理画面を開いてください。

3. 「Expo2025予約補助」の1回転矢印アイコンを押して拡張機能を再読み込みします。

4. すでに開いている万博のページの場合は、そのページをリロードしてください。
